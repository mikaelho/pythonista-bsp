...
class MarkdownWebView(Extender):
  ...
  def to_html(self, contents, caret_pos = 0, content_only = False):
    return self.generate_html(contents, self.htmlIntro, caret_pos, content_only)

  def to_web_html(self, contents):
    return self.generate_html(contents, self.web_intro, 0)

  def generate_html(self, contents, intro_template, caret_pos = 0, content_only = False):
    result = ''
    fragment = ''
    md = contents
    if caret_pos > 0:
  ...
  def update_html(self, contents, caret_pos = 0):
    self.last_contents = contents
    html = self.to_html(contents, caret_pos)
  ...
  def webview_should_start_load(self, webview, url, nav_type):
    ...
    if url.startswith(self.link_prefix):
      words = unquote(url.replace(self.link_prefix, ''))
      caret_pos = self.caret_pos_from_words(words)
    ...
    if url.startswith('/awz-'):
      words = self.eval_js('link_location()')
      caret_pos = self.caret_pos_from_words(words)
      target_key = url[5:]
      self.cross_link_func(target_key, caret_pos)
      return False
  ...
  def caret_pos_from_words(self, words):
    word_list = words.split()
    caret_pos = 0
    md = self.last_contents
  ...
  htmlIntro = Template('''
          ...
          <script type="text/javascript">
                  ...
                  function text_position() {
                          var e = window.event;
                          var data =  text_up_to_point(e.currentTarget, e.clientX, e.clientY);
                  ...
                  function link_location() {
                          ...
                          return text_up_to_point(elem, x, y)
                          //text_position_from_coords(elem, x, y);
                  }
